...
do
mkdir -p Results_t5_N638_SURFACE_24h/SURFACE_best_clade_2_id_$id
cd Results_t5_N638_SURFACE_24h/SURFACE_best_clade_2_id_$id
if [ -f "FinalResult_SURFACE_best_clade_2_id_"$id".RData" ]
then
rm MPI*.log
rm CurrentResults*.RData
else
bsub -n 10 -W 23:59 -R ib sh R --vanilla --slave -f ../../DetectShifts_t5_SURFACE_best_clade_2_24h.R --args $id
fi
cd ../..
done
