...
do
mkdir -p Results_t5_MGPM_A_F_best_clade_RR_N318/MGPM_A_F_best_clade_RR_id_$id
cd Results_t5_MGPM_A_F_best_clade_RR_N318/MGPM_A_F_best_clade_RR_id_$id
if [ -f "FinalResult_MGPM_A_F_best_clade_RR_id_"$id".RData" ]
then
rm MPI*.log
rm CurrentResults*.RData
else
bsub -M 100000 -n 20 -W 23:59 -R ib sh R --vanilla --slave -f ../../DetectShifts_t5_MGPM_A_F_best_clade_RR.R --args $id MGPM_A_F_best_clade_2_id_ ../../Results_t5_MGPM_A_F_best_clade_2_N318
fi
cd ../..
done
