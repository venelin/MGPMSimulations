...
do
mkdir -p Results_t5_MGPM_A_F_all_AIC2_N80/MGPM_A_F_all_AIC2_id_$id
cd Results_t5_MGPM_A_F_all_AIC2_N80/MGPM_A_F_all_AIC2_id_$id
if [ -f "FinalResult_MGPM_A_F_all_AIC2_id_"$id".RData" ]
then
rm MPI*.log
rm CurrentResults*.RData
else
R --vanilla --slave -f ../../ConvertToAIC2_t5_MGPM_A_F_all.R --args $id
fi
cd ../..
done
